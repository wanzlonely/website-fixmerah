...
const baileys=await import('@whiskeysockets/baileys');const {useMultiFileAuthState,makeWASocket,fetchLatestBaileysVersion,DisconnectReason}=baileys;
const pino=await import('pino').then(m=>m.default);const logger=pino({level:'silent'});
const {state,saveCreds}=await useMultiFileAuthState(WA_AUTH_DIR);
const {version}=await fetchLatestBaileysVersion();
...
try{sock.end()}catch{};return res.json({ok:true,results,count:results.length})
}
