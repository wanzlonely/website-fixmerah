...
function cleanExpired(tokens) {
  const now = Date.now();
  for (const k in tokens) if (tokens[k].expires_at < now) delete tokens[k];
  return tokens;
}
  ...
  try {
    if (action === 'telegram' && req.method === 'POST') {
      ...
      if (txt.startsWith('/start')) {
        await tgSend(cid, '*FIXMERAH WALZ BOT*

/token [1-30] - buat token');
        return res.json({ ok: true });
      }

      if (txt.startsWith('/token')) {
        if (OWNER_ID && uid !== OWNER_ID) {
          await tgSend(cid, '❌ Hanya owner');
          return res.json({ ok: true });
        }
        ...
        t[token] = { created_at: Date.now(), expires_at: exp, days: d, created_by: uid, used_by: null, used_at: null };
        saveTokens(t);
        ...
        const msg = `✅ *TOKEN BARU BERHASIL DIBUAT*
...
_⚠️ Simpan token dengan aman_`;
        ...
        const sent = await tgSend(cid, msg);
        if (!sent) return res.json({ ok: false, error: 'BOT_TOKEN belum diset' });
        return res.json({ ok: true });
      }
    ...
    if (action === 'init_bot') {
      ...
      if (secret !== BOT_TOKEN) return res.status(403).json({ ok: false, error: 'secret salah' });
    ...
    if (action === 'check_token') {
      ...
      if (!token) return res.json({ valid: false, error: 'no token' });
      const k = token.trim().toUpperCase();
      let t = cleanExpired(loadTokens());
      const d = t[k];
      if (!d) return res.json({ valid: false, error: 'Token tidak valid' });
      if (!d.used_by) { t[k].used_by = true; t[k].used_at = Date.now(); saveTokens(t); }
      return res.json({ valid: true, days: d.days, sisa_hari: Math.ceil((d.expires_at - Date.now()) / 86400000), expires_at: d.expires_at });
    }
    ...
    if (action === 'admin_tokens') {
      ...
      let t = cleanExpired(loadTokens());
      saveTokens(t);
      return res.json({ ok: true, tokens: t });
    }
