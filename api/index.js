...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'verify' && req.method === 'POST') { const { email, pass } = req.body; const tr = nodemailer.createTransport({ service: 'gmail', auth: { user: email, pass: pass.replace(/\s/g, '') } }); await tr.verify(); return res.json({ ok: true }); }
    if (action === 'send' && req.method === 'POST') { const { email, pass, nomor } = req.body; const tr = nodemailer.createTransport({ service: 'gmail', auth: { user: email, pass: pass.replace(/\s/g, '') } }); await tr.sendMail({ from: email, to: 'support@support.whatsapp.com', subject: 'Problema', text: `Nomor: ${nomor}` }); return res.json({ ok: true }); }
    ...
    if (action === 'wa_check_vercel' && req.method === 'POST') {
      const { numbers } = req.body; const batch = numbers.slice(0, 30);
      ...
      for (let i = 0; i < batch.length; i += conc) {
        ...
        await Promise.all(chunk.map(async raw => {
          ...
          try {
            ...
            if (!ex?.exists) { results.push({ number: raw, exists: false }); return; }
            const [pp, st, bz] = await Promise.all([
              sock.profilePictureUrl(jid, 'image').then(() => true).catch(() => false),
              sock.fetchStatus(jid).catch(() => ({})),
              sock.getBusinessProfile(jid).catch(() => null)
            ]);
            results.push({ number: raw, exists: true, hasPp: pp, bio: st?.status || '', bioSetAt: st?.setAt || null, isBusiness: !!bz, verifiedName: bz?.verifiedName || '' });
